...
import {
  ...
  DEFAULT_LAYOUT as ENGRAVING_LAYOUT_DEFAULTS,
  engravingLabelText,
      ...
      try {
        return await withSecurityScopedAccess(outputDirBookmark, async () => {
          ...
          function lwPolylineFlags(entity) {
            return (
              (polylineClosed(entity) ? 1 : 0) | (entity?.hasContinuousLinetypePattern ? 128 : 0)
            )
          }

          function polylineFlags(entity) {
            return (
              (polylineClosed(entity) ? 1 : 0) |
          ...
          function normalizeClosedPolylineVertices(vertices, closed) {
            if (!closed || !Array.isArray(vertices) || vertices.length < 2)
              return Array.isArray(vertices) ? vertices : []
            const last = vertices[vertices.length - 1]
            if (!Number.isFinite(last?.bulge) || last.bulge === 0) return vertices

            const zeroBulgeIndex = vertices.findIndex(
              (vertex) => !Number.isFinite(vertex?.bulge) || vertex.bulge === 0
            )
          ...
          function entityColorCodes(entity) {
            ...
            const aci = [entity.colorNumber, entity.colorIndex, entity.aci].find((value) =>
          ...
          function lineEndpoints(entity) {
            ...
            const start =
              entity.start ||
              (Array.isArray(entity.vertices) && entity.vertices.length >= 2
                ? entity.vertices[0]
                : null)
            const end =
              entity.end ||
          ...
          function joinConnectedLineworkEntities(entities) {
            ...
            groups.forEach((records) => {
              ...
              while (unused.size) {
                ...
                const seed: any = byId.get(seedId)
                ...
                while (degreeOf(cursorKey) === 2) {
                  ...
                  const nextRecord: any = byId.get(candidates[0])
          ...
          function dxfEntityOptions(entity): any {
            const color = entityColorCodes(entity)
            const options: any = { layerName: sanitizeDxfName(entity?.layer, '0') }
            ...
            if (
              ...
              options.extrusion = point3d(
                Number(extrusion.x || 0),
                Number(extrusion.y || 0),
                Number.isFinite(extrusion.z) ? Number(extrusion.z) : 1
              )
            }
            return options
          }
          ...
          function synthesizeEngravingLayer(layers, idx) {
            ...
            sourceLayers[idx - 1] = {
              name: `Layer ${idx}`,
              color:
                sourceLayers[idx - 1]?.color ||
                FALLBACK_LAYER_COLORS[(idx - 1) % FALLBACK_LAYER_COLORS.length]
            }
          ...
          function getEngravingLayer(item) {
            ...
            if (raw === 'off' || raw == null || raw === '' || (raw as any) === false) return null
          ...
          function addDxfEntity(dxf, entity, rotation, tx, ty, emitDebug: any = null) {
            if (!entity?.type) {
              if (emitDebug)
                emitDebug.skipped.push({ reason: 'missing-type', entity: entity || null })
              return false
            }
            ...
            if (entity.type === 'LINE') {
              ...
              if (!startPoint || !endPoint) {
                if (emitDebug) {
                  emitDebug.skipped.push({
                    ...
                    controlPointCount: Array.isArray(entity.controlPoints)
              ...
              dxf.addLine(
                point3d(start.x, start.y, start.z || 0),
                point3d(end.x, end.y, end.z || 0),
            ...
            if (
              ...
              const vertices = normalizedVertices.map((vertex) => {
                ...
                const entry: any = { point: point2d(point.x, point.y) }
              ...
              const flags =
                (polylineClosed(entity) ? LWPolylineFlags.Closed : LWPolylineFlags.None) |
            ...
            if (
              ...
              if (entity.is3dPolyline) {
                const vertices = normalizedVertices.map((vertex) => {
                  ...
                  const entry: any = { point: point3d(point.x, point.y, point.z || 0) }
                  if (Number.isFinite(vertex.startWidth)) entry.startingWidth = vertex.startWidth
                  if (Number.isFinite(vertex.endWidth)) entry.endWidth = vertex.endWidth
                  if (Number.isFinite(vertex.bulge)) entry.bulge = vertex.bulge
                  return entry
            ...
            if (
              entity.type === 'SPLINE' &&
              (entity.controlPoints?.length || entity.fitPoints?.length)
            ) {
          ...
          function buildDXF(
            ...
            if (exportSettings.useBlocks !== false) {
              ...
              sketchPlacements.forEach((placement) => {
                ...
                placement.entities.forEach((entity) =>
                  addDxfEntity(block, entity, 0, 0, 0, emitDebug)
                )
                definedBlocks.add(placement.blockName)
              })

              sketchPlacements.forEach((placement) => {
                dxf.modelSpace.addInsert(
                  placement.blockName,
                  point3d(placement.tx, placement.ty, 0),
                  {
              ...
              sheetEntities.forEach((entity) =>
                addDxfEntity(dxf, entity.entity, entity.rotation, entity.tx, entity.ty, emitDebug)
              )
            }
          ...
          for (let exportIndex = 0; exportIndex < strips.length; exportIndex++) {
            ...
            const sheetEntities: any[] = []
            const sketchPlacements: any[] = []
            const engravings: any[] = []
            const debugRows: any[] = []
            ...
            placedItems.forEach((placement) => {
              ...
              const placedHoles = Array.isArray(item.export?.holes)
                ? item.export.holes
                    .map((hole) =>
                      applyTransform(hole, rotation, tx, ty).map(([x, y]) => ({ x, y }))
                    )
              ...
              debugRows.push({
                ...
                export_layer_count: Array.isArray(exportItem?.layers)
                  ? exportItem.layers.length
                  : 0,
                export_entity_count: Array.isArray(exportItem?.entities)
                  ? exportItem.entities.length
                  : 0,
