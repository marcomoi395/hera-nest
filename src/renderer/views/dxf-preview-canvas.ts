...
function shapeDisplayScale(shape: any) {
...
export function createDxfPreviewCanvasView(deps: {
  ...
  function autoLayout(shapes: any[], canvasWidth: number) {
    let x = PAD
    let y = PAD
    let rowH = 0
  ...
  function buildPreviewSVG(shapes: any[], positions: any[], activeLayer: string | null, selectedId: string | null, canvasWidth: number) {
    ...
    const shapeEls = shapes.map((shape, index) => {
      ...
      const visibleDecorItems = layerMatch
        ? (shape.decorItems || []).filter((item: any) => !hasActiveLayer || item.layer === activeLayer)
        : []
      const visibleBoundaryItems = layerMatch
        ? (shape.outerBoundaryItems || []).filter((item: any) => !hasActiveLayer || item.layer === activeLayer)
        : []
      ...
      const normaliseStrokes = (svgStr: string) => svgStr.replace(
        /stroke-width="([^"]+)"/g,
        (_, v) => `stroke-width="${f(+v / displayScale)}"`
      )
      const strokeW = f(baseStrokeWidth / displayScale)
      const selStrokeW = f(2.8 / displayScale)
      const normBoundaryItems = visibleBoundaryItems.map((item: any) => normaliseStrokes(item.svg))
      const normDecorItems = visibleDecorItems.map((item: any) => normaliseStrokes(item.svg))
      return `
<g class="pvw-shape" data-id="${shape.id}"
  ...
  function renderSVG(onSelectShape: (id: string) => void) {
    ...
    if (canvasWrap) {
      ...
      canvasWrap.querySelectorAll('.pvw-shape').forEach(group => {
        group.addEventListener('click', () => onSelectShape((group as HTMLElement).dataset.id || ''))
      })
    }
  }
