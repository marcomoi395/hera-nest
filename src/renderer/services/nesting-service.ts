export function createNestingService(deps: {
  state: any
  dom: any
  getCurrentNestingSettings: () => any
  exportPlacementJSON: () => Promise<any>
  setStatus: (status: string) => void
  setNestStatsTone: (tone: string) => void
  showNestResult: (index: number) => void
  renderTabs: () => void
  syncExportButton: () => void
}) {
  const { state, dom, getCurrentNestingSettings, exportPlacementJSON, setStatus, setNestStatsTone, showNestResult, renderTabs, syncExportButton } = deps

  const MULTI_SHEET_STRATEGY_OPTIONS: Record<string, { multiStripMode: string; bucketFillWeight: number | null }> = (window as any).NestSettings?.MULTI_SHEET_STRATEGY_OPTIONS || {
  ...
  const scoreNestSummary = (window as any).NestResultScoring?.scoreNestSummary
  const isNestSummaryBetter = (window as any).NestResultScoring?.isNestSummaryBetter
  const shouldSkipTailRefinement = (window as any).NestTailRefinement?.shouldSkipTailRefinement
  const buildTailRefinementCandidates = (window as any).NestTailRefinement?.buildTailRefinementCandidates
  const buildTailSubsetPayload = (window as any).NestTailRefinement?.buildTailSubsetPayload
  const mergeTailReplacement = (window as any).NestTailRefinement?.mergeTailReplacement
  const scoreTailRefinementSummary = (window as any).NestTailRefinement?.scoreTailRefinementSummary
  const isTailRefinementBetter = (window as any).NestTailRefinement?.isTailRefinementBetter
  ...
  function showRunError(message: string, details = '') {
    setStatus('error')
  ...
  function showStartRequirementsWarning(message: string) {
    setStatus('idle')
  ...
  function qualityRunCount(settings: any): number {
    return Math.max(1, Math.min(5, Math.trunc(Number(settings.multiSeedQualityRuns) || 1)))
  }

  function qualityRunSeeds(settings: any, count: number): number[] {
  ...
  function tailRefinementEnabled(settings: any): boolean {
    return qualityRunCount(settings) > 1
  }

  function tailRefinementSeeds(settings: any): number[] {
    const count = Math.max(1, Math.min(5, Math.trunc(Number(settings.tailRuns) || 1)))
    return qualityRunSeeds(settings, count)
  }

  function tailRefinementOptions(baseOptions: any, settings: any): any {
    return {
      ...baseOptions,
      maxStripLength: null,
  ...
  async function waitForSparrowCompletion(runId: string): Promise<any> {
  ...
  async function runQualitySeedSequence(payload: any, baseOptions: any, settings: any): Promise<any> {
    ...
    for (const [index, seed] of seeds.entries()) {
      try {
        ...
        dom.nestStats.title = ''
        const result = await (window as any).electronAPI.runSparrow(payload, { ...baseOptions, rngSeed: seed })
        ...
        console.warn('[Sparrow] Quality seed failed:', seed, (err as any)?.sparrowDetails || err)
        try {
          await (window as any).electronAPI.stopSparrow?.()
        } catch (stopError) {
  ...
  async function runTailRefinement(baseSummary: any, payload: any, baseOptions: any, settings: any): Promise<any> {
    ...
    for (const candidate of candidates) {
      for (const seed of seeds) {
        ...
        try {
          const result = await (window as any).electronAPI.runSparrow(tailPayload, { ...options, rngSeed: seed })
          if (!result?.success || !result.runId) {
          ...
          console.warn('[Sparrow] Tail refinement candidate failed:', candidate.id, seed, (err as any)?.sparrowDetails || err)
          activeSparrowRunId = null
          try {
            await (window as any).electronAPI.stopSparrow?.()
          } catch (stopError) {
  ...
  async function pollSparrowRun(runId: string): Promise<any> {
    if (!(window as any).electronAPI?.pollSparrow) return

    const result = await (window as any).electronAPI.pollSparrow(runId)
    if (!result?.success) {
    ...
    if (result.status === 'error') {
      ...
      const err: any = new Error(extractSparrowErrorMessage(result.error, result.stderr, result.stdout))
  ...
  function bind() {
    ...
    dom.startBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const result = await (window as any).electronAPI.runSparrow(exported.payload, sparrowOptions)
    ...
    dom.stopBtn.addEventListener('click', async () => {
      ...
      activeSparrowRunId = null
      if ((window as any).electronAPI?.stopSparrow) {
        try {
          await (window as any).electronAPI.stopSparrow()
        } catch (err) {
