...
export function createDxfRasterEnvelopeService(deps: { geometry: any; shapeDetectionService?: any }) {
  const geometry = deps.geometry
  const debugDXF = deps.shapeDetectionService?.debugDXF || ((_label: string, _payload: any) => {})
...
export function buildRasterEnvelopes(entities: any[], options?: any): any[] {
  return []
}

export function detectRasterShapes(entities: any[], options?: any): any[] {
  return []
}
