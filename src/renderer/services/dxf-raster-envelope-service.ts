...
export interface GeometryService {
  ...
  closePointRing: (points: Point[]) => Point[]
  getLineEndpoints: (entity: unknown) => unknown
  polylineVerticesToPoints: (vertices: unknown[], close?: boolean) => Point[]
  ellipseToPoints: (entity: unknown, forceClosed?: boolean) => Point[]
  splineToPoints: (entity: unknown) => Point[]
  circleToPoints: (entity: unknown) => Point[]
  safeSamplePoint: (entity: unknown) => Point | null
  samePoint: (a: Point, b: Point, tolerance?: number) => boolean
...
export function createDxfRasterEnvelopeService(deps: {
  geometry: GeometryService
  shapeDetectionService?: ShapeDetectionService
}) {
  const geometry = deps.geometry
  const debugDXF =
    deps.shapeDetectionService?.debugDXF || ((_label: string, _payload: unknown) => {})
  ...
  function sampleArcPoints(entity: any) {
  ...
  function entityToPoints(entity: any) {
    if (!entity?.type) return []
    switch (entity.type) {
      case 'LINE': {
        const endpoints = getLineEndpoints(entity) as { start: Point; end: Point } | null
  ...
  function rasterizeEntities(grid, entities, options) {
    const strokeRadiusCells = Math.max(
      1,
      Math.round((options.strokeRadius || grid.cellSize * 1.2) / grid.cellSize)
    )
  ...
  function traceRegionPolygon(grid, region) {
    ...
    const polygon: any[] = [start]
    let currentKey: any = startKey
    let prevDir = { x: 1, y: 0 }
    let safety = 0
  ...
  function simplifyRegionBoundary(points, tolerance) {
    ...
    core.forEach((point) => {
      if (
        !simplified.length ||
        !samePoint(simplified[simplified.length - 1], point, tolerance * 0.25)
      ) {
  ...
  function assignEntitiesToRegions(grid, labels, regions, entities) {
    ...
    entities.forEach((entity) => {
      ...
      const bbox: any = entityBBox(entity)
      ...
      if (chosen && regionMap.has(chosen)) {
        ;(regionMap.get(chosen) as any).entities.push(entity)
      }
    })

    return [...regionMap.values()].filter((region: any) => region.entities.length)
  }

  function buildRasterEnvelopes(entities, options: any = {}) {
    ...
    const assignedRegions = assignEntitiesToRegions(grid, labels, regions, renderableEntities).map(
      (region: any, index) => {
...
// Stub exports for direct imports (will be replaced by factory instances)
export function buildRasterEnvelopes(entities: unknown[], options?: unknown): RasterEnvelope[] {
  return []
}

export function detectRasterShapes(entities: unknown[], options?: unknown): RasterEnvelope[] {
  return []
}
