import type { SettingsObject, MultiSheetStrategyConfig } from '../types/settings'

export const SKETCH_CONTOUR_METHODS = ['auto', 'arrangement'] as const

export const SHEET_WIDTH_PRIORITY_WEIGHTS: Record<string, number> = {
  'by-width': 0.0,
  'by-height': 1.0
}
...
export const MULTI_SHEET_STRATEGIES = [
  'auto',
  'by-height',
  'by-length',
  'by-height-or-length'
...
export const MULTI_SHEET_STRATEGY_OPTIONS: Record<string, MultiSheetStrategyConfig> = {
...
export const PREFERRED_ALIGNMENTS = [
  'top',
  'top-left',
  'top-right',
  'bottom',
...
export const SETTINGS_DEFAULTS: SettingsObject = {
  partSpacing: 0,
  sheetMargin: 0,
  rotationStep: '90',
  mirrorParts: false,
...
function coerceByDefault(value: unknown, fallback: unknown): unknown {
 ...
 */
export function normalizeSettings(input: unknown = {}): SettingsObject {
  const normalized: SettingsObject = { ...SETTINGS_DEFAULTS }
  ...
  if (!('multiSheetStrategy' in raw)) {
    const legacyBarrier =
      raw.useBarrierMode === true || raw.useBarrierMode === 'true' || raw.useBarrierMode === 1
    if (legacyBarrier) {
  ...
  Object.keys(SETTINGS_DEFAULTS).forEach((key) => {
    ...
    normalized[key] = coerceByDefault(raw[key], SETTINGS_DEFAULTS[key]) as never
  })
  ...
  if (
    ['makerjs-outline', 'makerjs-chains', 'intersection'].includes(normalized.sketchContourMethod)
  ) {
    normalized.sketchContourMethod = 'auto'
  }

  if (!PREFERRED_ALIGNMENTS.includes(normalized.preferredAlignment as never)) {
  ...
  if (!ENGRAVING_STYLES.includes(normalized.engravingStyle as never)) {
    normalized.engravingStyle = SETTINGS_DEFAULTS.engravingStyle
  }

  if (!SKETCH_CONTOUR_METHODS.includes(normalized.sketchContourMethod as never)) {
    normalized.sketchContourMethod = SETTINGS_DEFAULTS.sketchContourMethod
  }

  if (!MULTI_SHEET_STRATEGIES.includes(normalized.multiSheetStrategy as never)) {
  ...
  if (engravingLayerRaw !== 'off') {
    ...
    normalized.engravingLayer =
      Number.isFinite(parsed) && parsed >= 1 ? String(parsed) : SETTINGS_DEFAULTS.engravingLayer
  }
  ...
  normalized.multiSeedQualityRuns = Math.max(
    1,
    Math.min(5, Math.trunc(Number(normalized.multiSeedQualityRuns) || 1))
  )
  normalized.tailRuns = Math.max(1, Math.min(5, Math.trunc(Number(normalized.tailRuns) || 1)))
  normalized.rngSeed = Math.max(
    0,
    Math.trunc(Number(normalized.rngSeed) || SETTINGS_DEFAULTS.rngSeed)
  )
  normalized.workers = Math.max(
    1,
    Math.trunc(Number(normalized.workers) || SETTINGS_DEFAULTS.workers)
  )
